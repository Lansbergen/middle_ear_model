function [sample, CI95UCL, CI95LCL]=run_ttest_3mo(max_range,min_range)
...
change.ec_l = 1.15;     % length ear-canal cm
change.ec_d = 0.54;     % diameter ear-canal cm
...
change.fw = 450 * sample_factor(i);       % resonant frequency wall canal model
...
change.fw = 450;       % resonant frequency wall canal model
change.Qw = 2;        % Q-factor wall canal model
change.fCC = 1.25 * sample_factor(k);     % ratio Cw/Cm wall canal model
change.addC = 125;     % add acoustic ohm to reactance
change.fixR_onoff = 1; % 0=off,1=on
change.fixR = 120;     % fixed R-value       
...
change.ec_l = 1.15 * sample_factor(l);     % length ear-canal cm
change.ec_d = 0.54 * sample_factor(l);     % diameter ear-canal cm
change.mec_v = 4 * sample_factor(l);     % middle-ear cavity volume cc/ml/cm^3
change.mec_L = 0;      % middle-ear cavity L-value (Henry), when coupled -> n/a
change.mec_Lsw = 1;    % middle-ear cavity L-value coupled to volume on/off, 1=on
change.mec_R = 140 * sample_factor(l);     % middle-ear cavity resistance
change.onoff = 0;      % neglect wall admittance 1 = yes
change.fw = 450;       % resonant frequency wall canal model
change.Qw = 2;        % Q-factor wall canal model
change.fCC = 1.25;     % ratio Cw/Cm wall canal model
...
title('95% Confidence Bounds healthy 3 M.O. ear');
hold on
